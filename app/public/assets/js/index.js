...
// Emoji Events
var emojiElements = document.querySelectorAll('.emoji-popup .emoji-img')
...
(function ($s , Nodo , state) {
   ...
   class Chat2{
       ...
       static users(msgText){
           ...
           users.innerHTML = ""
           msg.users.splice( msg.users.indexOf(state.data.user), 1)
           if(msg.users.length){
   ...
   state.instanceDom()
   let mychat = new Chat()
   mychat.user = "learfen001@gmail.com"
   ...
   mychat.conversation("learfen001@gmail.com")
   
   Nodo.get("#login").addEventListener("click", socket.login );
   Nodo.get("#test").addEventListener("click", socket.test );

   // Button/Enter Key
   Nodo.get(".btn").addEventListener('click', mychat.send)
   mychat.inputChat.addEventListener('keyup', function(evt){ if(evt.keyCode == 13) mychat.send() })

   Nodo.get(".emoji-btn").addEventListener('click', function(e){
      e.stopPropagation()
      this.classList.toggle('open')
   })
